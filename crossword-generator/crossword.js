class CrosswordPuzzle {
    ...
    selectCell(index, forceToggle = false) {
        ...
        if (forceToggle && this.selectedClue !== null && cell.clues.includes(this.selectedClue) && cell.clues.length > 1) {
            ...
            if (this.selectedClue !== null) {
                ...
                targetClueIndex = cell.clues.find((clueIndex) => this.getClueDirection(clueIndex) === currentDirection) || cell.clues[0];
            } else {
    ...
    moveToNextWord() {
        ...
        if (nextWordClueIndex !== null) {
            ...
            const acrossClues = this.puzzle.clueLists.find((list) => list.name.toLowerCase() === 'across')?.clues || [];
            const downClues = this.puzzle.clueLists.find((list) => list.name.toLowerCase() === 'down')?.clues || [];
    ...
    isLastWordInDirection(clueIndex, direction) {
        const clueList = this.puzzle.clueLists.find((list) => list.name.toLowerCase() === direction.toLowerCase());
    ...
    updateMobileClueNavigatorPosition() {
        ...
        const isIOS =
            /iPad|iPhone|iPod/.test(window.navigator.userAgent) || (window.navigator.platform === 'MacIntel' && window.navigator.maxTouchPoints > 1);
    ...
    getNavigationState() {
        ...
        const currentDirection = this.getClueDirection(this.selectedClue);
        const currentClueList = this.puzzle.clueLists.find((list) => list.name.toLowerCase() === currentDirection.toLowerCase());
        ...
        const oppositeClueList = this.puzzle.clueLists.find((list) => list.name.toLowerCase() === oppositeDirection.toLowerCase());
...
class CrosswordLoader {
    ...
    showError(message) {
        ...
        document.getElementById(
            'crossword'
        ).innerHTML = `<div style="padding: 20px; text-align: center; color: #ff6b6b;">Failed to load puzzle. Please check the URL or try again later.</div>`;
