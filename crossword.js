class CrosswordPuzzle {
  ...
  selectCell(index, forceToggle = false) {
    ...
    if (
      ...
      if (this.selectedClue !== null) {
        ...
        targetClueIndex =
          cell.clues.find(clueIndex => this.getClueDirection(clueIndex) === currentDirection) ||
          cell.clues[0];
      } else {
  ...
  handleCellClick(index, event = null) {
    ...
    if (event && window.innerWidth <= 768) {
      ...
      setTimeout(() => {
        if (
          Math.abs((window.pageYOffset || document.documentElement.scrollTop) - currentScrollTop) >
          10
        ) {
          window.scrollTo({ top: currentScrollTop, behavior: 'instant' });
        }
      ...
      .catch(() => {
        ...
        if (leaderboardHeader) {
          leaderboardHeader.innerHTML =
            '🎉 Puzzle Complete! 🎉<br/><span style="font-size: 0.8em; font-weight: normal;">Showing current leaderboard...</span>';
        }
  ...
  moveToNextWord() {
    ...
    if (nextWordClueIndex !== null) {
      ...
      const acrossClues =
        this.puzzle.clueLists.find(list => list.name.toLowerCase() === 'across')?.clues || [];
      const downClues =
        this.puzzle.clueLists.find(list => list.name.toLowerCase() === 'down')?.clues || [];
  ...
  isLastWordInDirection(clueIndex, direction) {
    const clueList = this.puzzle.clueLists.find(
      list => list.name.toLowerCase() === direction.toLowerCase()
    );
      ...
      case 'Backspace':
        if (this.selectedClue !== null) {
          const currentCell = document.querySelector(
            `input.cell[data-index="${this.selectedCell}"]`
          );
  ...
  displayLeaderboardData(data) {
    ...
    if (!data || Object.keys(data).length === 0) {
      leaderboardBody.innerHTML =
        '<div class="empty-leaderboard">No times recorded yet today.<br/>Be the first to complete the puzzle!</div>';
  ...
  updateShareButtonVisibility() {
    ...
    if (shareSection) {
      ...
      const hasLeaderboardData =
        this.currentLeaderboardData && Object.keys(this.currentLeaderboardData).length > 0;
  ...
  updateMobileDynamicSizing() {
    ...
    const availableHeight =
      viewportHeight - titleAreaHeight - navigatorHeight - containerPadding - buffer;
    ...
    console.log(
      `Mobile sizing: ${cellSize}px cells (${gridWidth}x${gridHeight} grid) in ${availableWidth}x${availableHeight}px space`
    );
  }
  ...
  updateMobileClueNavigatorPosition() {
    ...
    if (isKeyboardVisible) {
      ...
      console.log(
        `Keyboard detected (${isIOS ? 'iOS' : 'Other'}): ${keyboardHeight}px, positioning navigator at bottom: ${bottomOffset}px`
      );
    } else {
  ...
  getNavigationState() {
    ...
    const currentDirection = this.getClueDirection(this.selectedClue);
    const currentClueList = this.puzzle.clueLists.find(
      list => list.name.toLowerCase() === currentDirection.toLowerCase()
    );
    ...
    const oppositeClueList = this.puzzle.clueLists.find(
      list => list.name.toLowerCase() === oppositeDirection.toLowerCase()
    );
...
class CrosswordLoader {
    ...
    if (this.crosswordInstance) {
      ...
      if (this.crosswordInstance.visibilityChangeHandler) {
        document.removeEventListener(
          'visibilitychange',
          this.crosswordInstance.visibilityChangeHandler
        );
  ...
  setupNavigation() {
    ...
    if (mobilePrevBtn)
      mobilePrevBtn.addEventListener('click', () => this.navigateToPreviousPuzzle());
  ...
  calculateTodaysPuzzleDate() {
    ...
    const puzzleDate = new Date(
      today.getFullYear() - this.yearOffset,
      today.getMonth(),
      today.getDate()
    );
